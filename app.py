...
# ============ LICENSE PROTECTION ============
try:
    from protection import LicenseProtection
    PROTECTION_AVAILABLE = True
    lp = LicenseProtection()
except ImportError:
    PROTECTION_AVAILABLE = False
    lp = None
    print("⚠️ Warning: protection.py not found. License protection disabled.")

# SINGLE license_required decorator
def license_required(f):
    """Decorator to protect routes with license check"""
    @wraps(f)
    def decorated_function(*args, **kwargs):
        if PROTECTION_AVAILABLE and lp:
            valid, message = lp.check_license()
            if not valid:
                if request.is_json:
                    return jsonify({'error': 'License expired or invalid. Please contact support.'}), 403
                flash("License expired or invalid. Please contact support.", "danger")
...
BACKGROUND_SETTINGS_FILE = os.path.join(BASE_DIR, 'background_settings.json')
...
def utility_processor():
    bg_settings = get_background_settings()
    return {
        'now': datetime.now,
        'background_image': bg_settings.get('background_image', ''),
        'background_style': bg_settings.get('background_style', 'cover'),
        'background_position': bg_settings.get('background_position', 'center'),
        'background_attachment': bg_settings.get('background_attachment', 'fixed'),
        'background_color': bg_settings.get('background_color', '#f0f2f5')
    }
...
class User(UserMixin, db.Model):
    ...
    role = db.Column(db.String(20), default='mda_user')
    name = db.Column(db.String(150))
    phone = db.Column(db.String(50), nullable=True)
    mda = db.Column(db.String(150), nullable=True)
    mda_attendance_link = db.Column(db.String(500), nullable=True)
...
class Employee(db.Model):
    ...
    registered_face_quality = db.Column(db.Float, default=0.0)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
...
def detect_and_encode_face(image_path):
    try:
        ...
        quality = min(face_size / img_size * 8, 1.0)
    ...
    with app.app_context():
        try:
            ...
            if 'updated_at' not in employee_columns:
                db.session.execute(text('ALTER TABLE employee ADD COLUMN updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP'))
                print("Added column to employee: updated_at")
            db.session.commit()
...
# ------------------------------
# License Error Page (ONLY ONE)
# ------------------------------
@app.route('/license_error')
def license_error():
    if lp:
        fingerprint = lp.get_hardware_fingerprint()
    else:
        fingerprint = "License protection not available"
    return render_template('license_error.html', fingerprint=fingerprint)
...
def detect_face_api():
    try:
        ...
        if os.path.exists(full_path):
            os.remove(full_path)
        return jsonify({'face_detected': face_detected, 'quality': round(quality, 2) if face_detected else 0})
    except Exception as e:
        print(f"Error: {e}")
        return jsonify({'face_detected': False}), 500
...
def debug_cropped_face():
    try:
        ...
        full_path = os.path.join(app.config['UPLOAD_FOLDER'], img_path)
        face_detected, encoding, location, quality = detect_and_encode_face(full_path)
        results = {'standard': {'detected': face_detected, 'quality': quality, 'location': location}}
        try:
            ...
            results['upsample'] = {'detected': len(face_locations) > 0, 'face_count': len(face_locations)}
        except Exception as e:
            results['upsample'] = {'error': str(e)}
        img = cv2.imread(full_path)
        if img is not None:
            results['image_info'] = {'shape': img.shape, 'min_dimension': min(img.shape[0], img.shape[1])}
        if os.path.exists(full_path):
            os.remove(full_path)
    ...
    if request.method == 'POST':
        ...
        if user and user.check_password(password):
            ...
            if user.role == 'superadmin':
                flash(f"👑 Welcome back Super Admin {user.name}!", "success")
            elif user.role == 'admin':
                flash(f"🛡️ Welcome back Admin {user.name}!", "success")
            elif user.role == 'mda_user':
                flash(f"🏢 Welcome back {user.name} from {user.mda}!", "success")
            else:
...
def register():
    if request.method == 'POST':
        ...
        if not mda:
            flash("❌ Your account doesn't have an MDA assigned. Contact super admin.", "danger")
        ...
        if face_match_employee:
            if not face_match_employee.is_active:
                ...
                flash(f"❌ Face already registered to active employee {face_match_employee.name} in {face_match_employee.mda}", "danger")
                return render_template('register.html', name=name, email=raw_email, phone=phone, mda=mda)
        if email:
            ...
            if existing_employee:
                if not existing_employee.is_active:
                    ...
                    flash(f"❌ Email {raw_email} already registered to active employee {existing_employee.name} in {existing_employee.mda}", "danger")
                    return render_template('register.html', name=name, email=raw_email, phone=phone, mda=mda)
        if phone:
            ...
            if existing_phone:
                if not existing_phone.is_active:
                    ...
                    flash(f"❌ Phone {phone} already registered to employee {existing_phone.name} in {existing_phone.mda}", "danger")
                    return render_template('register.html', name=name, email=raw_email, phone=phone, mda=mda)
        try:
            ...
            if b64:
                ...
                if img_filename:
                    full_path = os.path.join(app.config['UPLOAD_FOLDER'], img_filename)
                    face_detected, encoding, location, face_quality = detect_and_encode_face(full_path)
                    if not face_detected and os.path.exists(full_path):
                        os.remove(full_path)
...
# ------------------------------
# Take Attendance (Protected - Requires License)
# ------------------------------
...
def take_attendance():
    if request.method == 'POST':
        ...
        print(f"📸 [ATTENDANCE] Processing for employee: {empid}")
        if not empid:
        ...
        print(f"✅ [ATTENDANCE] Image saved: {img_path}")
        today = date.today()
        now_t = datetime.now().time()
        existing_timesheet = Timesheet.query.filter_by(employee_id=emp.id, date=today).first()
        if existing_timesheet and existing_timesheet.time_in and not existing_timesheet.time_out:
            existing_timesheet.signout_image = img_path
            existing_timesheet.time_out = now_t
            existing_timesheet.reg_signout_match = True
            existing_timesheet.signout_confidence = 1.0
            existing_timesheet.signout_face_quality = 1.0
            existing_timesheet.signout_liveness_passed = True
            db.session.commit()
            print(f"✅ [ATTENDANCE] {emp.name} signed OUT at {now_t}")
            return jsonify({"status": "ok", "action": "signed_out", "message": f"✅ {emp.name} signed OUT at {now_t.strftime('%I:%M %p')}"})
        else:
            if existing_timesheet:
                existing_timesheet.signin_image = img_path
                existing_timesheet.time_in = now_t
                existing_timesheet.reg_signin_match = True
                existing_timesheet.signin_confidence = 1.0
                existing_timesheet.signin_face_quality = 1.0
                existing_timesheet.signin_liveness_passed = True
            else:
                new_timesheet = Timesheet(
                    ...
                    date=today,
                    time_in=now_t,
                    reg_signin_match=True,
                    reg_signout_match=False,
                    signin_confidence=1.0,
                    signin_face_quality=1.0,
                    signin_liveness_passed=True
                )
                db.session.add(new_timesheet)
            db.session.commit()
            print(f"✅ [ATTENDANCE] {emp.name} signed IN at {now_t}")
            return jsonify({"status": "ok", "action": "signed_in", "message": f"✅ {emp.name} signed IN at {now_t.strftime('%I:%M %p')}"})
...
def live_attendance():
    if request.method == 'POST':
        ...
        if not match or confidence < 0.60:
            os.remove(captured_path)
            return jsonify({"status": "error", "msg": f"Face does not match registered employee. Confidence: {round(confidence*100)}% (need 60%)"}), 400
        today = date.today()
        ...
        if ts is None:
            ...
            db.session.add(new)
            db.session.commit()
            return jsonify({"status": "ok", "action": "signed_in", "face_match": match, "confidence": float(round(confidence * 100, 2)), "face_quality": float(round(face_quality * 100, 2)), "liveness_passed": bool(liveness_passed), "message": f"✅ LIVE ATTENDANCE: {emp.name} verified and signed in! ({round(confidence*100)}% match)"})
            ...
            db.session.commit()
            return jsonify({"status": "ok", "action": "signed_out", "face_match": match, "confidence": float(round(confidence * 100, 2)), "face_quality": float(round(face_quality * 100, 2)), "liveness_passed": bool(liveness_passed), "message": f"✅ LIVE ATTENDANCE: {emp.name} verified and signed out! ({round(confidence*100)}% match)"})
        else:
            os.remove(captured_path)
            return jsonify({"status": "error", "msg": "You have already completed attendance today (both signed in and out)."}), 400
    return render_template('live_attendance.html')

@app.route('/detect_face_liveness', methods=['POST'])
def detect_face_liveness():
    try:
        ...
        if len(faces) > 0:
            ...
            quality = min(1.0, max(0.3, brightness / 255.0))
            return jsonify({'face_detected': True, 'quality': float(quality), 'face_position': {'x': int(x), 'y': int(y), 'width': int(w), 'height': int(h)}})
        else:
            return jsonify({'face_detected': False, 'quality': 0})
    except Exception as e:
...
def api_employees():
    ...
    if search:
        query = query.filter(Employee.employeeid.ilike(f'%{search}%'))
    employees = query.limit(10).all()
    return jsonify([{'employeeid': e.employeeid} for e in employees])

@app.route('/api/attendance_status')
def api_attendance_status():
    employeeid = request.args.get('employeeid', '').strip().upper()
    emp = Employee.query.filter_by(employeeid=employeeid, is_active=True).first()
    if not emp:
        return jsonify({'error': 'Employee not found'}), 404
    today = date.today()
    timesheet = Timesheet.query.filter_by(employee_id=emp.id, date=today).first()
    ...
    sign_out_time = None
    if timesheet:
        sign_in_time = format_time(timesheet.time_in)
        sign_out_time = format_time(timesheet.time_out)
    result = {
        'name': emp.name,
        'employeeid': emp.employeeid,
        'sign_in': timesheet is not None and timesheet.time_in is not None,
        'sign_out': timesheet is not None and timesheet.time_out is not None,
        'sign_in_time': sign_in_time,
        'sign_out_time': sign_out_time,
        'has_completed': timesheet is not None and timesheet.time_in is not None and timesheet.time_out is not None
    }
    if result['has_completed']:
        result['message'] = f'You have already signed in and out for today. Signed in at {result["sign_in_time"]}, Signed out at {result["sign_out_time"]}.'
    elif result['sign_in']:
        result['message'] = 'Currently signed in. Taking photo will sign you out.'
    else:
        result['message'] = 'Ready to sign in.'
    return jsonify(result)

@app.route('/api/employee_exists')
def api_employee_exists():
    employeeid = request.args.get('employeeid', '')
    emp = Employee.query.filter_by(employeeid=employeeid, is_active=True).first()
    return jsonify({'exists': emp is not None})
...
def match_face():
    try:
        ...
        if best_match and best_confidence >= 0.60:
            return jsonify({'match_found': True, 'employee': {'employeeid': best_match.employeeid, 'name': best_match.name, 'mda': best_match.mda}, 'confidence': round(best_confidence * 100, 2)})
        ...
        for emp in employees:
            ...
            if os.path.exists(reg_path):
                ...
                if reg_encoding is not None:
                    ...
                    if match and confidence > 0.7:
                        os.remove(full_path)
                        return jsonify({'is_duplicate': True, 'existing_employee': emp.name, 'is_active': emp.is_active, 'employee_mda': emp.mda})
        os.remove(full_path)
        return jsonify({'is_duplicate': False})
    except Exception as e:
...
def employees():
    ...
    for emp in items:
        ...
        employee_data.append({'employee': emp, 'user_account': user, 'has_user': user is not None, 'user_role': user.role if user else None})
    total_count = len(employee_data)
    active_count = sum(1 for emp in employee_data if emp['employee'].is_active)
    inactive_count = total_count - active_count
    return render_template('employees.html', employees=employee_data, user_role=current_user.role, current_mda=current_user.mda, show_mda_filter=show_mda_filter, all_mdas=all_mdas, filter_mda=request.args.get('mda', ''), total_count=total_count, active_count=active_count, inactive_count=inactive_count, now=datetime.now())
...
def edit_employee(id):
    ...
    if request.method == 'POST':
        ...
        if remove_image and emp.registered_image:
            old_image_path = os.path.join(app.config['UPLOAD_FOLDER'], emp.registered_image)
            if os.path.exists(old_image_path):
                os.remove(old_image_path)
            emp.registered_image = None
            emp.registered_face_quality = 0
            flash("✅ Employee photo removed", "success")
        if 'employee_image' in request.files:
            ...
            if file and file.filename:
                ...
                if file_ext not in allowed_extensions:
                    flash("❌ Invalid file type. Please upload JPG, JPEG, or PNG images only.", "danger")
                ...
                filename = f"employee_{emp.employeeid}_{timestamp}.{file_ext}"
                ...
                try:
                    ...
                    if img.mode not in ['RGB', 'RGBA']:
                        ...
                        rgb_img.paste(img, mask=img.split()[3] if len(img.split()) > 3 else None)
                        img = rgb_img
                    jpeg_filename = f"employee_{emp.employeeid}_{timestamp}.jpg"
                    jpeg_filepath = os.path.join(app.config['UPLOAD_FOLDER'], jpeg_filename)
                    img.save(jpeg_filepath, 'JPEG', quality=95)
                    if jpeg_filename != filename:
                        if os.path.exists(filepath):
                            os.remove(filepath)
                        filename = jpeg_filename
                        filepath = jpeg_filepath
                    face_detected, encoding, location, face_quality = detect_and_encode_face(filepath)
                    if face_detected:
                        emp.registered_image = filename
                        emp.registered_face_quality = face_quality
                        flash(f"✅ Photo uploaded successfully! Face quality: {round(face_quality*100)}%", "success")
                    else:
                        emp.registered_image = filename
                        emp.registered_face_quality = 0
                        flash("⚠️ Photo uploaded but no face detected. Please upload a clear face photo for better recognition.", "warning")
                except Exception as e:
        ...
        if user_action == 'create':
            ...
            if user:
                ...
                return render_template('edit_employee.html', emp=emp, user_role=current_user.role, has_user_account=existing_user is not None)
            try:
                user = User(email=emp.email, password_hash=generate_password_hash(new_password), name=emp.name, phone=emp.phone, mda=emp.mda, role='user', employee_id=emp.id)
                db.session.add(user)
            ...
            try:
                existing_user_check = User.query.filter(User.email == emp.email, User.id != user.id).first()
...
def export_mismatch_pdf():
    ...
    if date_filter:
        try:
            ...
            pass
    q = q.filter(or_(Timesheet.reg_signin_match == False, Timesheet.reg_signout_match == False))
...
def export_excel():
    ...
    rows = []
    for t in items:
        rows.append({"employee_name": t.employee_name, "mda": t.mda, "date": t.date.isoformat(), "time_in": t.time_in.isoformat() if t.time_in else "", "time_out": t.time_out.isoformat() if t.time_out else "", "signin_confidence": round(t.signin_confidence*100, 2) if t.signin_confidence else "", "signout_confidence": round(t.signout_confidence*100, 2) if t.signout_confidence else "", "face_match_status": "✅ Match" if t.reg_signin_match and t.reg_signout_match else "❌ Mismatch"})
...
def office():
    ...
    office_employees = []
    try:
        query = db.session.query(Timesheet, Employee).outerjoin(Employee, Timesheet.employee_id == Employee.id).filter(Timesheet.date == today, Timesheet.time_in.isnot(None), Timesheet.time_out.is_(None))
        if not current_user.is_admin() and current_user.mda:
        ...
        for ts, emp in employees_query:
            office_employees.append({'id': ts.id, 'employee_name': ts.employee_name or (emp.name if emp else 'Unknown'), 'mda': ts.mda or (emp.mda if emp else 'N/A'), 'time_in': ts.time_in, 'signin_image': ts.signin_image, 'reg_signin_match': ts.reg_signin_match, 'signin_confidence': ts.signin_confidence, 'Employee': emp})
    except Exception as e:
...
def dashboard():
    ...
    q = Timesheet.query
    if current_user.is_superadmin() or current_user.is_admin():
        if 'mda_filter' in locals() and mda_filter:
    ...
    for t in items:
        ...
        if name not in stats:
            stats[name] = {"mda": existing_employees.get(name, t.mda), "total_days": 0, "signed_in_count": 0, "signed_out_count": 0, "signed_in_no_signout": 0, "early_count": 0, "late_count": 0, "ontime_count": 0, "left_early_count": 0, "stayed_till_close_count": 0, "exact_timeout_count": 0, "avg_confidence": 0, "total_confidence": 0, "high_confidence_count": 0, "medium_confidence_count": 0, "low_confidence_count": 0, "avg_face_quality": 0, "total_face_quality": 0, "liveness_passed_count": 0, "liveness_failed_count": 0, "records": []}
        s = stats[name]
        s["records"].append(t)
        s["total_days"] += 1
        if t.time_in:
    ...
    for name, v in stats.items():
        ...
        stats_list.append({"employee_name": name, "mda": v["mda"], "total_days": v["total_days"], "signed_in_count": v["signed_in_count"], "signed_out_count": v["signed_out_count"], "signed_in_no_signout": v["signed_in_no_signout"], "early_count": v["early_count"], "late_count": v["late_count"], "ontime_count": v["ontime_count"], "punctuality_rate": round(punctuality_rate, 1), "left_early_count": v["left_early_count"], "stayed_till_close_count": v["stayed_till_close_count"], "exact_timeout_count": v["exact_timeout_count"], "stay_rate": round(stay_rate, 1), "attendance_rate": round(attendance_rate, 1), "avg_confidence": round(avg_confidence * 100, 2), "high_confidence_count": v["high_confidence_count"], "medium_confidence_count": v["medium_confidence_count"], "low_confidence_count": v["low_confidence_count"], "avg_face_quality": round(avg_face_quality * 100, 2), "liveness_passed_count": v["liveness_passed_count"], "liveness_failed_count": v["liveness_failed_count"], "liveness_rate": round((v["liveness_passed_count"] / (v["liveness_passed_count"] + v["liveness_failed_count"]) * 100), 1) if (v["liveness_passed_count"] + v["liveness_failed_count"]) > 0 else 0, "trace_link": url_for('employee_trace', name=name)})
    stats_list.sort(key=lambda x: x["total_days"], reverse=True)
    summary = {'total_employees': total_employees, 'total_attendance_records': total_attendance_records, 'active_today': sum(1 for t in items if t.date == date.today() and t.time_in and not t.time_out), 'completed_today': sum(1 for t in items if t.date == date.today() and t.time_in and t.time_out), 'avg_attendance_rate': round(sum(s['attendance_rate'] for s in stats_list) / len(stats_list), 1) if stats_list else 0, 'avg_punctuality_rate': round(sum(s['punctuality_rate'] for s in stats_list) / len(stats_list), 1) if stats_list else 0, 'avg_stay_rate': round(sum(s['stay_rate'] for s in stats_list) / len(stats_list), 1) if stats_list else 0}
    return render_template('dashboard.html', stats=stats_list, user_role=current_user.role, current_mda=current_mda, mda_context=mda_context, all_mdas=all_mdas, summary=summary, now=datetime.now())
...
def onboard_mda():
    if request.method == 'POST':
        ...
        if User.query.filter_by(email=email).first():
            ...
            return render_template('onboard_mda.html')
        try:
            user = User(name=name, email=email, phone=phone, mda=mda, role='mda_user', password_hash=generate_password_hash(phone))
            db.session.add(user)
            db.session.commit()
...
def admin_users():
    ...
    query = User.query
    if search:
        query = query.filter(db.or_(User.name.ilike(f'%{search}%'), User.email.ilike(f'%{search}%'), User.mda.ilike(f'%{search}%')))
...
def admin_add_user():
    ...
    if request.method == 'POST':
        ...
        try:
            user = User(name=name, email=email, phone=phone, mda=mda, role=role, password_hash=generate_password_hash(password), employee_id=employee.id if employee else None)
            db.session.add(user)
            db.session.commit()
            if employee:
...
def admin_edit_user(id):
    ...
    if request.method == 'POST':
        ...
        mda_attendance_link = request.form.get('mda_attendance_link', '').strip()
        if not all([name, email, phone, mda]):
        ...
        try:
            ...
            user.mda_attendance_link = mda_attendance_link if mda_attendance_link else None
            if new_password:
                user.password_hash = generate_password_hash(new_password)
                credentials = f"Username: {email}\nPassword: {new_password}\nMDA: {mda}\nMDA Attendance Link: {mda_attendance_link if mda_attendance_link else 'Not provided'}\nRole: {role}\nEmployee ID: {link_employee if link_employee else 'Not linked'}\nUpdated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
                user.credentials = credentials
                flash(f"🔑 Password updated for {user.email}", "success")
            else:
                credentials = f"Username: {email}\nPassword: [Current password unchanged]\nMDA: {mda}\nMDA Attendance Link: {mda_attendance_link if mda_attendance_link else 'Not provided'}\nRole: {role}\nEmployee ID: {link_employee if link_employee else 'Not linked'}\nLast Updated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
                user.credentials = credentials
            db.session.commit()
...
def signup():
    if request.method == 'POST':
        ...
        employee = Employee.query.filter_by(email=email).first()
        u = User(email=email, password_hash=generate_password_hash(pwd), role='user', name=name, mda=mda, employee_id=employee.id if employee else None)
        db.session.add(u)
        db.session.commit()
        if employee:
    ...
    if request.method == 'POST':
        if 'background_image' in request.files:
            ...
            if file and file.filename:
                ...
                file_ext = file.filename.rsplit('.', 1)[1].lower() if '.' in file.filename else ''
                if file_ext not in allowed_extensions:
                    flash("❌ Invalid file type. Please upload JPG, PNG, GIF, or WEBP images only.", "danger")
                    return redirect(url_for('admin_background'))
                bg_dir = os.path.join(app.config['UPLOAD_FOLDER'], 'backgrounds')
                os.makedirs(bg_dir, exist_ok=True)
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                filename = f"background_{timestamp}.{file_ext}"
        ...
        elif request.form.get('use_existing'):
            filename = request.form.get('use_existing')
            settings = get_background_settings()
            settings['background_image'] = filename
            save_background_settings(settings)
            flash("✅ Background image applied!", "success")
        settings = get_background_settings()
        settings['background_style'] = request.form.get('background_style', 'cover')
        settings['background_position'] = request.form.get('background_position', 'center')
        settings['background_attachment'] = request.form.get('background_attachment', 'fixed')
        settings['background_color'] = request.form.get('background_color', '#f0f2f5')
        save_background_settings(settings)
        flash("✅ Background settings saved!", "success")
        return redirect(url_for('admin_background'))
    settings = get_background_settings()
    current_bg = settings.get('background_image', '')
    current_style = settings.get('background_style', 'cover')
    current_position = settings.get('background_position', 'center')
    current_attachment = settings.get('background_attachment', 'fixed')
    current_color = settings.get('background_color', '#f0f2f5')
    bg_dir = os.path.join(app.config['UPLOAD_FOLDER'], 'backgrounds')
    available_bgs = []
    if os.path.exists(bg_dir):
        for file in os.listdir(bg_dir):
            if file.lower().endswith(('.jpg', '.jpeg', '.png', '.gif', '.webp')):
                available_bgs.append(file)
    return render_template('admin_background.html', current_bg=current_bg, current_style=current_style, current_position=current_position, current_attachment=current_attachment, current_color=current_color, available_bgs=available_bgs)

@app.route('/admin/background/delete/<filename>')
@login_required
@ip_whitelist
def admin_background_delete(filename):
    if not current_user.is_superadmin():
        flash("👑 Super Admin access required!", "danger")
        return redirect(url_for('dashboard'))
    try:
        ...
        if os.path.exists(filepath):
            os.remove(filepath)
        settings = get_background_settings()
        if settings.get('background_image') == filename:
            settings['background_image'] = ''
            save_background_settings(settings)
        flash(f"✅ Background image {filename} deleted!", "success")
    except Exception as e:
...
if __name__ == '__main__':
    with app.app_context():
        ...
        if not superadmin:
            superadmin = User(name='sadmin', email='sadmin@gmail.com', password_hash=generate_password_hash('sadmin123'), role='superadmin', mda='SYSTEM')
            db.session.add(superadmin)
            db.session.commit()
            print("✅ Super Admin created. Email: sadmin@gmail.com Password: sadmin123")
        admin = User.query.filter_by(email='admin@gmail.com').first()
        if not admin:
            admin = User(name='Administrator', email='admin@gmail.com', password_hash=generate_password_hash('admin123'), role='admin', mda='ADMIN')
            db.session.add(admin)
            db.session.commit()
            print("✅ Admin created. Email: admin@gmail.com Password: admin123")
        print("\n🔑 User Roles:")
