...
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///edovoice.db'
        ...
        if path:
            ...
            if os.path.exists(fs_path):
                try:
                    ...
                    print(f"Failed to delete file {fs_path}: {e}")
...
def update_status(report_id):
    ...
    if report.status == 'Completed' and new_status in ['Pending', 'In Progress']:
        if report.resolutionVideoUrl:
            video_path = report.resolutionVideoUrl.lstrip('/')
            if os.path.exists(video_path):
                try:
                    os.remove(video_path)
                    print(f"🗑️ Deleted resolution video: {video_path}")
                except Exception as e:
...
if __name__ == '__main__':
    with app.app_context():
        ...
        # ===== UPDATED: Add password reset columns to users table =====
        try:
    ...
    print('🔐 Password reset: http://127.0.0.1:5000/forgot-password')
    app.run(debug=True)